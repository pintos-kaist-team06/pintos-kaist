...
/* syscall_handler() 가 제어권을 잡으면, system call num= %rax, arguments = [%rdi, %rsi, %rdx, %r10, %r8, %r9] 순서로 전달된다.
...
__attribute__((always_inline)) static __inline int64_t syscall(uint64_t num_, uint64_t a1_, uint64_t a2_, uint64_t a3_, uint64_t a4_, uint64_t a5_, uint64_t a6_) {
    ...
    __asm __volatile(
        ...
        "syscall\n"
        : "=a"(ret)
        : "g"(num), "g"(a1), "g"(a2), "g"(a3), "g"(a4), "g"(a5), "g"(a6)  // 인자 설정
        : "cc", "memory");
    return ret;
}

/*syscall-nr에서 받아온 enum값을 가지고 함수 실행 */
