...
tid_t process_create_initd(const char *file_name) {
    char *fn_copy;  // file_name과 load() 경쟁 피하기위해 복사
    tid_t tid;
...
int process_exec(void *f_name) {
    ...
    /*스레드 리스케쥴링될때 원래 스레드 정보 없어지니까 새 _if를 선언
    만약 다른 작업(예: 인터럽트 처리)에서 스레드 구조체 내의 intr_frame을 사용하게 되면, 그 정보가 변경될 위험이 있습니다. 이는 스레드가 재개될 때 원래의 상태를 복원하는 데
    문제. 따라서, 현재 진행 중인 스레드의 intr_frame을 사용하는 것이 아니라, 새로운 intr_frame 인스턴스를 사용*/
    struct intr_frame _if;

    /* 데이터, 스택, 코드, 추가세그먼트를 USER영역 세그먼트로 설정*/
